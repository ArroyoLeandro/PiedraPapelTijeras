...
function convertCase(anythingIwant) {
  ...
  if (anythingIwant === 'scissors') return 'Tijeras';
  return 'Piedra';
}
...
function win(user, computer) {
  ...
  result_div.innerHTML = `<p>${convertCase(user)}${userName} Es mejor que ${convertCase(computer)}${compName}. Tu Ganas!</p>`;
...
function loses(user, computer) {
  ...
  const userName = ' (user)'.fontsize(3).sup();
  const compName = ' (comp)'.fontsize(3).sup();
  result_div.innerHTML = `<p>${convertCase(computer)}${compName}  Es mejor que ${convertCase(user)}${userName}. Tu Pierdes!</p>`;
  const roundStatus = document.getElementById(user);
...
function draw(user, computer) {
	const userName = ' (user)'.fontsize(3).sup();
  const compName = ' (comp)'.fontsize(3).sup();
  result_div.innerHTML = `<p>Es un Empate! Ambos Eligieron  ${convertCase(user)}</p>`;
